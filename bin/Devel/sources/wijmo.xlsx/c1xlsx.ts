...
module wijmo.xlsx {
	...
	export function _xlsx(file): any {
        ...
        var result, zip = new JSZip(), zipTime, processTime, s, content, f, i, j, k, l, t, w, sharedStrings, styles, index, data, val, formula /* GrapeCity: Add formula variable.*/, sharedFormulas /* GrapeCity: Add sharedFormulas variable.*/, cellRef /* GrapeCity: Add cellRef variable.*/, si /* GrapeCity: Add si(shared Index) variable.*/, rowStyle /* GrapeCity: Add rowStyle variable.*/, columnStyle /* GrapeCity: Add columnStyle variable.*/,
            sheetStyle /* GrapeCity: Add sheetStyle variable.*/, style, borders, fonts, font,
            docProps, xl, xlWorksheets, worksheet, worksheetVisible /* GrapeCity: Add worksheetVisible variable.*/, contentTypes = [], contentType, props = [], xlRels = [], xlRel, worksheets = [], sheetEle, id, columns, cols, columnSettings, colWidth, pane, cell, row, merges, idx, colIndex, groupLevel, frozenRows, frozenCols, fills, fill, macroEnabled /* GrapeCity: Add macroEnabled variable. */, applicationType /* GrapeCity: Add applicationType variable. */, summaryBelow,
            ...
            indexedColors = ['000000', 'FFFFFF', 'FF0000', '00FF00', '0000FF', 'FFFF00', 'FF00FF', '00FFFF',
                ...
                '000000', 'FFFFFF'],
            domParser, xmlSerializer, xmlDescription = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>', sheetDoc, sheet, sheetData, col, pageMargins,
            docRegExp = /xmlns\:NS\d+=\"\"\s+NS\d+\:/g,
		...
		function getColor(s, isFillColor) {
            var theme, index, value,
                color = isFillColor ? s.querySelector('fgColor') : s.querySelector('color');
            if (!color) {
                return null;
            }
            if (color.hasAttribute('rgb')) {
                ...
                index = +color.getAttribute('indexed');
		...
		function getsBaseSharedFormulas(sheet: any) {
            var formulas = sheet.querySelectorAll('sheetData>row>c>f[ref]'),
				formula: any,
				sharedIndex: string,
				cellRef: string;
        ...
        function generateRelsDoc(): Document {
        ...
        function generateThemeDoc(): Document {
        ...
        function generateClrScheme(doc: Document): Element {
        ...
        function generateFontScheme(doc: Document): Element {
        ...
        function generateFmtScheme(doc: Document): Element {
        ...
        function generateFillScheme(doc: Document): Element {
        ...
        function generateLineStyles(doc: Document): Element {
        ...
        function generateEffectScheme(doc: Document): Element {
        ...
        function generateBgFillScheme(doc: Document): Element {
        ...
        // Generate core doc.
        function generateCoreDoc(): Document {
            var doc = document.implementation.createDocument('', '', null);
        ...
        function generateSheetGlobalSetting(sheetDoc: Document, worksheet: any) {
        ...
        function generateCell(sheetDoc: Document, rowIndex: number, colIndex: number, styleIndex: number, type: string, val: any, formula: string): Element {
        ...
        function generateMergeSetting(sheetDoc: Document, merges: any[]): Element {
        ...
        // Generate style doc
        function generateStyleDoc(): Document {
        ...
        function generateBorderStyle(doc: Document, borders: any): Element {
        ...
        function generateFontStyle(doc: Document, fontStyle: any, needScheme: boolean = false): Element {
        ...
        function generateFillStyle(doc: Document, patternType: string, fillColor: string): Element {
        ...
        // Generate xf element
        function generateCellXfs(doc: Document, numFmtId: number, borderId: number, fontId: number, fillId: number, style: any): Element {
        ...
        // Generate app doc
        function generateAppDoc(): Document {
        ...
        function generateWorkbookRels(): Document {
            var doc = document.implementation.createDocument(relationshipsNS, 'Relationships', null);
        ...
        function generateWorkbook(): Document {
            ...
            doc.documentElement.appendChild(calcPr); '</sheets><calcPr fullCalcOnLoad="1"/></workbook>';
            return doc;
        }

        // Generate shared strings doc.
        function generateSharedStringsDoc(): Document {
			...
			if (s = zip.file('xl/styles.xml')) { // Get style info from "xl/styles.xml"
                ...
                while (i >= 0) {
                    ...
                    styles.unshift({
                        ...
                        font: font,
                        fillColor: fill,
                        hAlign: s[i].querySelector('alignment') != null ? Workbook._parseStringToHAlign(s[i].querySelector('alignment').getAttribute('horizontal')) : null,
                        wordWrap: s[i].querySelector('alignment') != null ? s[i].querySelector('alignment').getAttribute('wrapText') === '1' : null,
                    });
                    i--;
                }
			}
