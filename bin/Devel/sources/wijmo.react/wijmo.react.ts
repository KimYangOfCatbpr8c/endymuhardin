...
module wijmo.react {
     ...
     */
    export var BulletGraph = React.createClass({
        render: function () {
            return React.createElement('div');
        },
        componentDidMount: function () {
     ...
     */
    export var PivotPanel = React.createClass({
        render: function () {
            return React.createElement('div');
        },
        componentDidMount: function () {
            _mount(this, wijmo.olap.PivotPanel);
        },
        componentWillUnmount: function () {
            _unmount(this);
        },
     ...
     */
    export var PivotGrid = React.createClass({
        render: function () {
            return React.createElement('div');
        },
        componentDidMount: function () {
            _mount(this, wijmo.olap.PivotGrid);
        },
        componentWillUnmount: function () {
            _unmount(this);
        },
     ...
     */
    export var PivotChart = React.createClass({
        render: function () {
            return React.createElement('div');
        },
        componentDidMount: function () {
            _mount(this, wijmo.olap.PivotChart);
        },
        componentWillUnmount: function () {
            _unmount(this);
        },
    ...
    function _mount(component, controlType): any {
        ...
        for (var prop in cprops) {
            if (prop in control) {
                ...
                switch (prop) {
                    case 'className':
                        addClass(host, cprops.className);
                        break;
                    case 'style':
                        setCss(host, cprops.style);
                        break;
    ...
    function _updateControl(component, nextProps): boolean {
    ...
    function _copy(dst, src) {
        for (var p in src) {
            ...
            if (p in src && p != 'itemsSource' && !_sameValue(dst[p], value)) {
                ...
                if (p == 'className') {
                    if (dst.hostElement) {
                        addClass(dst.hostElement, src[p]);
                    }
                } else if (p == 'style') {
                    if (dst.hostElement) {
                        setCss(dst.hostElement, src[p]);
                    }
