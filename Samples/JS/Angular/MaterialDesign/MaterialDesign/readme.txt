MaterialDesign
---------------------------
Shows how to use Wijmo with Google's Material Design Lite.
...
which we use extensively in our Wijmo samples.
...
download the minified CSS. Wijmo includes 286 Material themes that represent
all valid combinatios of primary and accent colors.
...
1) It monitors MDL tab elements and invalidates Wijmo controls when the 
   user switches tabs. This allows Wijmo controls to update their layout
   when they become visible.

2) It adds event handlers for Wijmo input elements contained in MDL TextField
   elements to update their state attributes (focus, dirty, invalid). This
   allows you to use Wijmo input elements in MDL TextFields in exactly the
   same way you would use native input elements.
